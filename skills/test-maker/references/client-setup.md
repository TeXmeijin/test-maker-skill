...
> **`gh skill` を使う前提**: GitHub CLI **v2.90.0 以降** が必要（[リリース情報](https://github.com/cli/cli/releases/tag/v2.90.0)）。現在は **public preview** でコマンド仕様は変更の可能性あり。古いバージョンしか無い場合は `brew upgrade gh` 等で更新するか、後述の手動 clone を使う。
...
**方法1: `gh skill` CLI（クロスエージェント・要 gh v2.90.0+）**
